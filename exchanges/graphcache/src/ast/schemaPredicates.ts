  ...
  for (const givenMutation of givenMutations) {
    if (schemaMutations.indexOf(givenMutation) === -1) {
      warn(
        'Invalid mutation: `' +
          givenMutation +
          '` is not a mutation in the defined schema, but the `updates` option is referencing it.',
        21
      );
    }
  }
    ...
    if (schemaSubscriptions.indexOf(givenSubscription) === -1) {
      warn(
        'Invalid subscription: `' +
          givenSubscription +
          '` is not a subscription in the defined schema, but the `updates` option is referencing it.',
        22
      );
    }
  }
